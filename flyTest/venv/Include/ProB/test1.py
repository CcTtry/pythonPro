...
excel1 = pd.read_excel("附件一：325个样本数据.xlsx", sheet_name='Sheet1',
                       header=None, names=ids1, skiprows=3)
for i in range(16, N+16):
    excel1[ids1[i]] = excel1[ids1[i]].values.astype(float)
    excel1[ids1[i]] = excel1[ids1[i]].values.astype(float)

coordinates_1 = excel1.values[:, 16:]  # 全部行 16列  不包含第7列
# 保存了所有对应列标签的范围
starts = {}   # np.zeros((len(coordinates_4)))  # 根据ID存储范围起点
ends = {}
print(len(excel1))  # 325

for i in range(16, N + 16):
    starts[ids1[i]] = excel1[ids1[i]].min()
    ends[ids1[i]] = excel1[ids1[i]].max()

# 至此 范围已经得到 可以使用starts[“列标签”]  、 ends[“列标签”] 的形式读取
# for i in range(16, 16 + N):   # N = 354
#     print(" {}范围是   {}-{}".format(ids1[i], starts[ids1[i]], ends[ids1[i]]))

ids = list(excel4.values[:, 1])   # 获取列标签  ids1 和 ids不同
